...
| HBOX_SPEECH_BASE_URL                    |                                                                                                | API root of an OpenAI-compatible speech-to-text provider used for voice input, e.g. `https://api.mistral.ai/v1`, `https://api.openai.com/v1`, or `https://api.groq.com/openai/v1` (scheme required; no query string). Voice input is disabled unless both this and HBOX_SPEECH_MODEL are set and the URL is valid — misconfiguration is logged at startup. Note: browsers only expose the microphone in secure contexts, so the web UI must be served over HTTPS (or accessed via localhost) for the mic button to appear. |
| HBOX_SPEECH_MODEL                       |                                                                                                | Transcription model identifier at the configured provider, e.g. `voxtral-mini-transcribe` (Mistral), `gpt-4o-mini-transcribe` (OpenAI), or `whisper-large-v3-turbo` (Groq).               |
| HBOX_SPEECH_API_KEY                     |                                                                                                | API key for the speech-to-text provider. The key stays server-side; audio is proxied through the HomeBox backend. Optional for unauthenticated providers on a private network.             |
| HBOX_SPEECH_INTENT_MODEL                |                                                                                                | Optional chat model with function-calling support at the same provider (e.g. `mistral-small-latest`, `gpt-4o-mini`). Setting it enables voice *commands* ("add two batteries to the garage shelf") on top of dictation: the transcript is mapped to a proposed action that prefills the create form for you to confirm. Leave unset to keep voice input dictation-only. |
| HBOX_SPEECH_LANGUAGE                    |                                                                                                | Optional ISO-639-1 language code to pin transcription to a single language. Leave unset to let the model auto-detect.                                                                     |
| HBOX_SPEECH_TIMEOUT                     | 30s                                                                                            | Timeout for a single transcription request to the provider. Clamped below HBOX_WEB_WRITE_TIMEOUT (the response cannot be written after that deadline) — raise both to allow slower providers. Voice clip uploads are capped by HBOX_WEB_MAX_FILE_UPLOAD.  |
